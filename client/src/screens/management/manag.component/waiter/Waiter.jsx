 ...
 const fetchInternalOrders = async () => {
  ...
  const updateOrderDelivered = async (id) => {
    try {
      ...
      const updateOrder= await axios.put(`https://calma-api-puce.vercel.app/api/order/${id}`, { status ,products});
  ...
  const helpOnWay = async (id) => {
    try {
      ...
      const res=await axios.put(`https://calma-api-puce.vercel.app/api/order/${id}`, { helpStatus });
      if(res.status==200){
        fetchInternalOrders();
        fetchPendingData();
  ...
  const helpDone = async (id) => {
    try {
      ...
      await axios.put(`https://calma-api-puce.vercel.app/api/order/${id}`, { helpStatus });
      fetchPendingData();
      fetchInternalOrders();
